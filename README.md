...
1. Clone the git repo — `git` from <branch>

2. Then:
- Copy the files to your project into a directory like `/Automation` (note the hidden files!)
...
4. Create your branch and work on it don't work on master

5. Install the dependencies (`npm install`)

Now you are ready to write your own features.

6. From VSCode extensions install Cucumber (Gherkin) Full Support follow steps within to enable
...
Below are different ways to run a test if baseUrl & tagExpression is not defined it will pick the default mentioned in wdio.config.js
Note: Browser has to be defined for execution they can me ',' separated and scripts will be executed based on the list provided

1. npm run spec -- --caps=chrome,firefox
2. npm run suite <suite name> -- --caps=chrome,firefox --baseUrl='<url>'
3. npm run <suite|spec> -- --caps=chrome --baseUrl='<url>' --cucumberOpts.tagExpression="@<tag name>"
